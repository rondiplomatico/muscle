...
\begin{align}
	...
	g(\lambda_f)&:= \frac{b_1}{\lambda_f^2}\left(\lambda_f^{d_1} - 1\right)
...
\begin{align*}
	...
	I_1(\vC) &= \tr(\vI_3) = 3, & I_4(\vC,\va_0) & = \vI_3 : \va_0(X)\otimes\va_0(X)\\
		...
		\lambda_f &= 1, & \gamma(\alpha,\lambda_f,\lambda_f) &= 0,
\end{align*}
Hence we obtain from \eqref{def:overallS}
\begin{align}
	0 &= \vS(X,0) = p(X,0)\vI + 2(c_{10} + 3c_{01})\vI - 2c_{01}\vI\\
			 &\quad+\left(\frac{b_1}{1}\left(1 - 1\right)+\frac{p^{max}}{1}\gamma\left(0,1,0\right)\right)\va_0(X)\otimes\va_0(X)\nonumber\\
