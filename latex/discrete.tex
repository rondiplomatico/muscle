We now collect the $N$ coefficients $\vc_i$ as $\vu(t) = (\vc_1^T(t) \ldots \vc_N^T(t))^T \in\R^{3N}$
...
\begin{align}
	%\Rightarrow \d{}{\vU}\vS^{-1} &= \lim_{h\to 0}\frac{(\vS+h\vU)^{-1}-\vS^{-1}}{h} = \lim_{h\to 0}\frac{\vS^{-1} - \vS^{-1}h\vU\vS^{-1}-\vS^{-1}}{h}\nonumber\\
	%	&= -\vS^{-1}\vU\vS^{-1}\nonumber\\
...
% \begin{align*}
% 	\d{\vF}{\vu[k]}(X,\vu) &= \nabla_{\vu[k]}\vF(X,\vu)\\
% 			&= \nabla_{\vu[k]}\sumi \vu[i] \otimes \divergence\varphi_i(X) = \bullet\otimes\divergence\varphi_k(X)\\
% 	\nabla_{\vu[k]}(\det\vF(X,\vu)-1) &= \det(\vF(X,\vu))\vF(X,\vu)^{-1}:\nabla_{\vu[k]}\vF(X,\vu)\\
% 				&= \det(\vF(X,\vu))\vF(X,\vu)^{-1}:\bullet\otimes\divergence\varphi_k(X)
% \end{align*}
